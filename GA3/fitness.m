function chroms = fitness(chroms, Gate, Flight)
%% 该函数用于计算chroms的适应度。
%{
% 适应度分为三部分：登机口利用率
...
while chromsIndex<=n
    ZCProbability = sum(chroms{1,chromsIndex}.unappropriated)/m;
    chroms{1,chromsIndex}.fitness1 = ZCProbability;
    chromsIndex = chormsIndex+1;
end
%}
%% 登机口利用率计算
chromsIndex = 1;    % 针对每个染色体进行计算
